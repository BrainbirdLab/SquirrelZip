...
func Zip(files []utils.File) (utils.File, error) {
	...
	// Create buffer to store compressed content
...
func Unzip(file utils.File) ([]utils.File, error) {
	...
	decompressedContent, err := decompressData(compressedContent, &root)
	if err != nil {
		return nil, err
	}
...
func decompressData(data []byte, root *Node) ([]byte, error) {
	var buf bytes.Buffer
	...
	node := root
	for _, b := range data {
		err := decompressByte(b, &buf, &node, root)
		if err != nil {
			return nil, err
		}
	}
...
func decompressByte(b byte, buf *bytes.Buffer, node **Node, root *Node) error {
	for i := 7; i >= 0; i-- {
		bit := (b >> i) & 1
		if bit == 0 {
			*node = (*node).left
		...
		if (*node).left == nil && (*node).right == nil {
			err := buf.WriteByte(byte((*node).char))
			if err != nil {
				return err
			}
			*node = root
